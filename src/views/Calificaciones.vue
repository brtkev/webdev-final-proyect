...
<template>
  <h1 class="view-title">
    <span style="color: white"
      ><i class="fa-solid fa-school-flag"></i> Manejo de Calificaciones</span
    >
  </h1>
  <Form
    :headers="headers"
    ...
    @update-click="onUpdateCalificacionesClick"
  ></Form>
</template>
...
import {
  ...
  estudiantes,
  refreshEstudiantes,
  estudianteIsDisabled,
  asignaturasIsDisabled,
  onUpdateEstudianteModel,
  afterRequest,
  onUpdateClick,
} from "../components/viewHandlers/HandleEstudiantesYAsignaciones.js";
...
function onUpdateCalificacionesEstudianteModel(id) {
  ...
  asignaturas.value = asignaturas.value.filter((item) =>
    matriculacionesData.some(
      (matricula) =>
        matricula.estudiante == id && matricula.asignaturas.includes(item.value)
    )
  );
}
...
const headers = [
  {
    ...
    items: estudiantes,
    isMultiple: false,
    onUpdateModelValue: onUpdateCalificacionesEstudianteModel,
    isDisabled: estudianteIsDisabled,
  },
  {
    title: "Asignatura",
    key: "asignatura",
    ...
    items: asignaturas,
    isMultiple: false,
    isDisabled: asignaturasIsDisabled,
  },
  {
    title: "Calificacion",
    key: "calificacion",
...
export default {
  ...
  setup() {
    ...
    /** refresca los hooks */
    Promise.all([refreshAsignaturas(), refreshEstudiantes()]).then(async () => {
      ...
      estudiantes.value = estudiantes.value.filter((item) =>
        matriculacionesData.some(
          (matricula) => item.value == matricula.estudiante
        )
      );
    });
