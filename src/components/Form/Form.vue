  ...
  <v-app class="form-container">
    ...
    <div v-for="(header, i) in headers" :key="i" style="margin-bottom: 1.5rem">
      <v-select
        ...
        :items="header.items"
        :label="header.title"
        :multiple="header.isMultiple"
        :disabled="header.isDisabled"
        @update:modelValue="header.onUpdateModelValue"
      ></v-select>
      <v-number-input
    ...
    <v-container v-else class="update-buttons-container">
      <v-btn
        class="form-button form-update-button form-save-button"
        style="margin-right: 1.5rem"
        @click="saveUpdatedItem"
        >Actualizar</v-btn
      >
      <v-btn
        class="form-button form-update-button form-update-cancel-button"
        @click="cancelUpdatedItem"
        >Cancelar</v-btn
      >
    </v-container>
...
export default {
    ...
    headers: {
      ...
      validator: (value) => {
        return value.every((item) => {
          const required =
            item.hasOwnProperty("title") && item.hasOwnProperty("key");
          if (item.isArray) {
            return (
              required && item.hasOwnProperty("items") && isRef(item.items)
            );
          } else {
            return required;
          }
  ...
  setup(props) {
    ...
    const addItem = async () => {
      ...
      try {
        const { status, message } = await validateBeforeRequest({
          entity: props.entity,
          data: data.value,
        });
    ...
    const cancelUpdatedItem = () => {
      ...
      const dataIndex = tableData.findIndex(
        (data) => data.id == updateOriginalData.id
      );
